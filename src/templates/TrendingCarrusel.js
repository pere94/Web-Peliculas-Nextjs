...
function TrendingCarrusel() {
    const { trendingMovies, setTrendingMovies, trendingTvShows, setTrendingTvShows, trendingActors, setTrendingActors } = React.useContext(AppContext);
    ...
    React.useEffect(() => {
        const getTrending = async () => {
            if (trendingMode === 'TV') {
                ...
                setTrendingTvShows(trendingTvShowsPage == 1 ? trendingItems.results : trendingTvShows.concat(trendingItems.results));
            } else if (trendingMode === 'PERSON') {
                const trendingItems = await GetApi(endPointsApi.trending.person(trendingActorsPage));
                setTrendingActors(trendingActorsPage == 1 ? trendingItems.results : trendingActors.concat(trendingItems.results));
            } else {
                const trendingItems = await GetApi(endPointsApi.trending.movie(trendingMoviesPage));
                setTrendingMovies(trendingMoviesPage == 1 ? trendingItems.results : trendingMovies.concat(trendingItems.results));
            }
        };
        getTrending();
        ...
        <div className="  ">
            ...
            >
                ...
                <div className="flex gap-3">
                    <TrendingButton text="Movies" changeTrendingMode={() => setTrendingMode('MOVIE')} mode="MOVIE" trendingMode={trendingMode} />
                    <TrendingButton text="TV Shows" changeTrendingMode={() => setTrendingMode('TV')} mode="TV" trendingMode={trendingMode} />
                    <TrendingButton text="Actors" changeTrendingMode={() => setTrendingMode('PERSON')} mode="PERSON" trendingMode={trendingMode} />
                </div>
            </div>
                ...
                {trendingMode === 'MOVIE' &&
                    trendingMovies.map((item) => {
                        ...
                        if (!idRenderedArray.includes(item.id)) {
                            ...
                            return (
                                <MovieCard
                                    imgCard={item.poster_path ? `https://image.tmdb.org/t/p/w440_and_h660_face/${item.poster_path}` : userDontImg}
                                    title={item.title}
                ...
                {trendingMode === 'TV' &&
                    trendingTvShows.map((item) => {
                        ...
                        if (!idRenderedArray.includes(item.id)) {
                            ...
                            return (
                                <MovieCard
                                    imgCard={item.poster_path ? `https://image.tmdb.org/t/p/w220_and_h330_face/${item.poster_path}` : userDontImg}
                                    title={item.name}
                ...
                {trendingMode === 'PERSON' &&
                    trendingActors.map((item) => {
                        ...
                        if (!idRenderedArray.includes(item.id) && item.known_for_department == 'Acting' && item.profile_path != null) {
                            ...
                            return (
                                <PersonCard
                                    imgCard={item.profile_path ? `https://www.themoviedb.org/t/p/w470_and_h470_face/${item.profile_path}` : userDontImg}
                                    name={item.name}
