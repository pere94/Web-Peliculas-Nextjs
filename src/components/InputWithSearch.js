...
function InputWithSearch({label, inputRef}) {
    ...
    return ( 
        <>
            ...
            <input ref={inputRef} onChange={handleInputChange} id="contSearcher__filtros-actors" list="actors" placeholder="Actor Name" 
            ...
            <datalist id="actors">
                {actorsList.map( item => (
                    <option key={'Actor-'+item.id} value={item.id}>{item.name}</option>
                ))
                }
            </datalist>
        </>
