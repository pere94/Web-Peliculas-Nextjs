...
function PersonCard(props) {
    const { imgCard, name, role, popularity, id } = props;

    return (
        <>
            <Link href={`/person/${name.toLowerCase().replace(/:/g, '').replace(/ /g, '-')}?id=${id}`}>
                <article className="w-52 min-w-[200px] flex flex-col p-1 rounded-2xl h-[360] hover:opacity-100">
                    <div href="#" className="no-underline rounded-2xl overflow-hidden cursor-pointer max-h-[300px]">
                        ...
                        <div
                            className={` relative -top-[306px] opacity-0 py-2 px-3 text-xl font-medium text-white bg-gray-900/80  
                    ...
                    <div>
                        <p className="text-sm text-version1 dark:text-version3 pl-2">{role}</p>
                        <h3 title={name} id="h3Trend0" className="truncate mx-2 text-version2 dark:text-gray-50 h-6">
                            {name}
                        </h3>
                    </div>
                    <div className="text-sm text-version1 dark:text-version3 flex items-center justify-end gap-1 px-1 py-1">
                        <div className="flex items-center">
                            <p id="voteAverage0">{popularity}</p>
